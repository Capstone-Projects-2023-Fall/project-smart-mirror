...
export default function SettingsPage() {
  ...
  /*For toggle check*/
  const LocationToggle = (checked: boolean) => setLocationEnabled(checked);
  const TrackingToggle = (checked: boolean) => setTrackingEnabled(checked);
  const AnalyticsToggle = (checked: boolean) => setAnalyticsEnabled(checked);
  const [mirrorAlwaysOn, setMirrorAlwaysOn] = useState(false);
  const [sleepTimer, setSleepTimer] = useState("30"); // Example default value
  /*--------MirrorSetting--------*/
  const handleMirrorAlwaysOnToggle = (checked: boolean) => {
  ...
  /*----------------------------------------------------------------------------------------------*/
  const UsernameChange = (event: React.FocusEvent) => {
    /* ... */
  };
  const PasswordChange = (event: React.FocusEvent) => {
    /* ... */
  };
  const EmailChange = (event: React.FocusEvent) => {
    /* ... */
  };
  const BirthdayChange = (event: React.FocusEvent) => {
    /* ... */
  };
  const GenderChange = (event: React.FocusEvent) => {
    /* ... */
  };
