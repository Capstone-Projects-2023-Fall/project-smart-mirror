  ...
  const fetchWidgetVisibility = async () => {
    try {
      const response = await fetch(
        "http://localhost:1023/api/widget-visibility",
      );
      if (!response.ok) {
      ...
      console.error(
        "There was an error fetching the widget visibility:",
        error,
      );
    }
  };
      ...
      {Object.keys(widgetVisibility).map((widgetName) => (
        <div key={widgetName} className="toggle-switch-container">
          ...
          <label className="toggle-switch">
            <input
              type="checkbox"
              checked={widgetVisibility[widgetName as keyof WidgetVisibility]}
              onChange={() =>
                toggleWidgetVisibility(widgetName as keyof WidgetVisibility)
              }
